...
export function CitySearch({ snapshot, city }: { snapshot: GitHubYearSnapshot; city: CityModel }) {
  ...
  return (
    <div className="pointer-events-auto relative w-64">
      ...
      {matches.length > 0 && <div className="absolute left-0 right-0 top-10 border border-[var(--line)] bg-[#111419] p-1 shadow-2xl">{matches.map((match) => <button key={match.id} type="button" onClick={() => { sceneStore.selectLot(match.id); setQuery(match.label); }} className="flex w-full justify-between px-2 py-2 text-left font-[family-name:var(--font-mono)] text-[8px] tracking-[0.1em] text-white hover:bg-[var(--surface-high)]"><span>{match.label}</span><span className="text-[var(--muted)]">{match.type}</span></button>)}</div>}
    </div>
  );
}
