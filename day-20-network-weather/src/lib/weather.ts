...
export function classifyNetworkWeather(
  metrics: NetworkMetrics,
): NetworkWeather {
  if (metrics.totalProbes > 0 && metrics.failedProbes === metrics.totalProbes)
    return "offline";
...
export function weatherReasons(metrics: NetworkMetrics) {
  ...
  if (metrics.failedProbes > 0) {
    reasons.push(
      `${metrics.failedProbes} HTTP ${
        metrics.failedProbes === 1 ? "probe failed" : "probes failed"
      }.`,
    );
  }
  return reasons;
